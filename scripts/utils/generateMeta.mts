...
const releases = await fetch("https://api.github.com/repos/BashOnZsh/Bashbop/releases", {
    headers: {
...
const metaInfo = await (async () => {
    for (const release of releases) {
        const metaAsset = release.assets.find((a: any) => a.name === "org.bashonzsh.bashbop.metainfo.xml");
...
await fs.writeFile("./dist/org.bashonzsh.bashbop.metainfo.xml", output, "utf-8");
console.log("Updated meta information written to ./dist/org.bashonzsh.bashbop.metainfo.xml");
